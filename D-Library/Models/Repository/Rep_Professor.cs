...
namespace D_Library.Models.Repository
{
    public class Rep_Professor
    {
        ...
        public IEnumerable<SelectListItem> Get_AllProfessorSelectList()
        {
            ...
            foreach (var item in q)
            {
                ...
                if (p != null)
                {
                    ...
                    {
                       // t.Add(new DropDownModel((int)master.Login_UserID, master.Tbl_User.User_Name + " " + master.Tbl_User.User_Family));
                        list.Add(new SelectListItem() { Value = master.Login_UserID.ToString(), Text = master.Tbl_User.User_Name + " " + master.Tbl_User.User_Family });
                    }
